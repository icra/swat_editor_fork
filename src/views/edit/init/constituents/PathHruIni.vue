<template>
	<project-container :loading="page.loading" :load-error="page.error">
		<file-header input-file="path_hru.ini" docs-path="initialization-data">
			Pathogens (HRU)
		</file-header>
		...
		<b-alert :show="constituents.length < 1" variant="primary">
			You must first enable the <router-link to="/edit/init/constituents">constituents file</router-link> and add pathogens before editing this section.
		</b-alert>
			...
			<b-form @submit="save" :validated="page.validated">
				<b-table-simple responsive small table-class="edit border-bottom">
					...
					<b-tbody v-for="(item, i) in items" :key="i">
						...
						<b-tr>
							<b-td>Amount of pathogen on plant at start of simulation (kg/ha)</b-td>
							<b-td v-for="(c, j) in constituents" :key="j">
								<b-form-input type="number" step="any" v-model.number="items[i].rows[j].plant" maxLength="16" />
							</b-td>
							<b-td></b-td>
						</b-tr>
						<b-tr>
							<b-td>Amount of pathogen in soil at start of simulation (kg/ha)</b-td>
							<b-td v-for="(c, j) in constituents" :key="j">
								<b-form-input type="number" step="any" v-model.number="items[i].rows[j].soil" maxLength="16" />
							</b-td>
							<b-td></b-td>
						</b-tr>
					</b-tbody>
...
<script>
export default {
	name: 'InitConstituentsPathHruIni',
	data() {
		return {
			apiUrl: 'constituents/path-hru',
			relatedKey: 'path_hrus',
			page: {
				loading: false,
				error: null,
				validated: false,
