<template>
	<project-container :loading="page.loading" :load-error="page.error">
		<file-header input-file="plant_gro.sft" docs-path="change-calibration">
			<router-link to="/edit/change/soft">Soft Calibration</router-link>
			/ <router-link to="/edit/change/soft/plant">Plant Growth</router-link>
			/ Edit
		</file-header>
		...
		<regions-form is-update lum-lookup
			...
			:api-url="apiUrl"
			redirect-route="SoftCalibrationPlantGroEdit" />
	</project-container>
</template>
...
export default {
	name: 'SoftCalibrationPlantGroEdit',
	components: {
		RegionsForm
	},
	data() {
		return {
			...
			apiUrl: 'change/soft/regions/plant',
			item: {},
			vars: [],
			initItems: []
		}
	...
	methods: {
		async get() {
            ...
            try {
				...
				const response2 = await this.$http.get(`vars/plant_gro_sft/${this.appPath}`);
