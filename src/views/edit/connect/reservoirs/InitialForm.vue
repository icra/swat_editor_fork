	...
	methods: {
		putDb(data) {
			if (this.page.bulk.show)
				return this.$http.put('reservoirs/initial/many/' + this.projectDbUrl, data);
			else if (this.isUpdate)
				return this.$http.put('reservoirs/initial/' + this.item.id + '/' + this.projectDbUrl, data);
			else
				return this.$http.post('reservoirs/initial/post/' + this.projectDbUrl, data);
		},
		async save() {
            ...
            if (!val_error) {
                try {
                    ...
                    if (this.isUpdate)
                        ...
                        this.$router.push({ name: 'ReservoirsInitial'});
                } catch (error) {
