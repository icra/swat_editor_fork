		...
		<b-form :validated="page.validated" @submit.prevent="save">
			...
			<b-row>
				<b-col md="6">
					<b-form-group>
						<template slot="label">
							<b-form-checkbox v-if="page.bulk.show" v-model="selectedVars" value="dlr_name" class="d-inline"></b-form-checkbox>
							Delivery Ratio Properties
							<font-awesome-icon :icon="['fas', 'question-circle']" class="text-info pointer" id="dlr_name_pop" />
						</template>
						<b-popover target="dlr_name_pop" triggers="hover focus">
							<reference-label section="Routing Units / Delivery Ratio" file="rout_unit.dr" db="rout_unit_dr" />
						</b-popover>

						<type-ahead-and-go :show-button="isUpdate"
							v-model="item.props.dlr_name"
							:value="item.props.dlr_name"
							table-name="rtu_dr" route-name="#"></type-ahead-and-go>
					</b-form-group>
				</b-col>
			</b-row>
...
export default {
	...
	data() {
		return {
			redirectRoute: 'RoutingUnitEdit',
			propCol: 'rtu',
			propFields: [
				'topo_name', 'dlr_name', 'field_name'
			],
			page: {
				error: null,
				validated: false,
