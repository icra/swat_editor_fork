<template>
	<project-container :loading="page.loading" :load-error="page.error">
		<file-header input-file="initial.aqu" docs-path="connections/aquifers">
			...
			<router-link to="/edit/aquifers/initial">Initial</router-link>
			/ Edit
		</file-header>
...
export default {
	...
	methods: {
		async get() {
			...
			try {
				const response = await this.$http.get(`aquifers/initial/${this.$route.params.id}/${this.projectDbUrl}`);
				...
				this.page.error = this.logError(error, 'Unable to get aquifer properties from database.');
			}
