...
class Poll_hru_ini(base.BaseModel):
	name = CharField(unique=True)

class Poll_hru_ini_item(base.BaseModel):
	poll_hru_ini = ForeignKeyField(Poll_hru_ini, on_delete='CASCADE', related_name="poll_hrus")
	name = ForeignKeyField(hru_parm_db.Pollutants_pth, on_delete='CASCADE', null=True)
	plant = DoubleField()
	soil = DoubleField()
