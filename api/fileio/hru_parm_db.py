...
#ICRA Adrià Riu
class Pollutants_pth(BaseFileModel):
	...
	def read(self, database ='project'):
		if database == 'project':
			self.read_default_table(project_parmdb.Pollutants_pth, project_base.db, 0, ignore_id_col=True)
		else:
			self.read_default_table(datasets_parmdb.Pollutants_pth, datasets_base.db, 0, ignore_id_col=True)
