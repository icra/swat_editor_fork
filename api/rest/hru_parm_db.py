...
""" ICRA Adrià Riu Database - Pollutants"""

class PollutantsPthListApi(BaseRestModel):
	def get(self, project_db):
		table = hru_parm_db.Pollutants_pth
		filter_cols = [table.name]
		return self.base_paged_list(project_db, table, filter_cols)


class PollutantsPthApi(BaseRestModel):
	def get(self, project_db, id):
		return self.base_get(project_db, id, hru_parm_db.Pollutants_pth, pollutant_name)

	def delete(self, project_db, id):
		return self.base_delete(project_db, id, hru_parm_db.Pollutants_pth, pollutant_name)

	def put(self, project_db, id):
		return self.base_put(project_db, id, hru_parm_db.Pollutants_pth, pollutant_name)


class PollutantsPthUpdateManyApi(BaseRestModel):
	def get(self, project_db):
		return self.base_name_id_list(project_db, hru_parm_db.Pollutants_pth)

	def put(self, project_db):
		return self.base_put_many(project_db, hru_parm_db.Pollutants_pth, pollutant_name)


class PollutantsPthPostApi(BaseRestModel):
	def post(self, project_db):
		return self.base_post(project_db, hru_parm_db.Pollutants_pth, pollutant_name)


class PollutantsPthDatasetsApi(BaseRestModel):
	def get(self, datasets_db, name):
		return self.base_get_datasets_name(datasets_db, name, ds.Pollutants_pth, pollutant_name)
